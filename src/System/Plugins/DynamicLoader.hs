...
data DynamicModule = RTM
  { dm_qname :: [String]
  , dm_path :: FilePath
  }

data DynamicPackage = RTP
  { dp_path :: FilePath
...
loadModuleFromPath path mbase =
    ...
    let name =
          reverse $
            drop 1 $
              dropWhile (/= '.') $
                reverse $
                  if head qual == '/' then drop 1 qual else qual
  ...
  where
    dropIsEq [] ys = return ys
    dropIsEq (x : xs) (y : ys)
      | x == y = dropIsEq xs ys
      | otherwise =
          fail $
            "Unable to get qualified name from: "
              ++ path
...
loadPackage name mpath mpre msuff =
  do
    ...
    if cbitsExist
      then do
        rtp <- loadPackage (name ++ "_cbits") mpath mpre msuff
...
loadPackageFromPath path =
  do
    ...
    if cbitsExist
      then do
        rtp <- loadPackageFromPath cbits_path
        return (RTP path (Just rtp))
      else return (RTP path Nothing)
  where
    cbitsName :: FilePath -> String
    cbitsName name =
      let suffix = reverse $! takeWhile (/= '.') rname
          rname = reverse name
       in reverse (drop (length suffix + 1) rname)
            ++ "_cbits."
            ++ suffix -- wrong but simple...
