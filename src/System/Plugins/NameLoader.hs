...
midLoadModule (Just sm) _ _ =
  return $
    ( sm{sm_refc = sm_refc sm + 1}
    , sm_deps sm
    )
...
loadFunction crit (LM m) name =
  ...
  where
    loadFunction' (_, MT_Package) _ _ = fail "Cannot load functions from packages"
    loadFunction' (mname, _) fname (_, _, _, _, _, _, modh) =
      do
        msm <- HT.lookup modh mname
        sm <-
          maybe
